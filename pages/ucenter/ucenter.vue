<template>
	...
	<view class="fixbg" v-if="imageURL && imageURL!=''" :style="{ 'background-image' : 'url(' + imageURL + ')' }">
		<view class="center">
			...
			<view class="userInfo">
				<view class="logo-image" @click.capture="toUserInfo">
					<image v-if="userInfo.avatar_file&&userInfo.avatar_file.url"
						:src="userInfo.avatar_file.url"></image>
					<image v-else src="@/static/uni-center/defaultAvatarUrl.png"></image>
				</view>
...
<!-- 			<uni-grid class="grid" :column="4" :showBorder="false" :square="true">
			...
			<uni-list v-else class="center-list" v-for="(sublist , index) in ucenterList" :key="index">
				<uni-list-item v-for="(item,i) in sublist" :title="item.title" link :rightText="item.rightText" :key="i"
					:clickable="true" :to="item.to" @click="ucenterListClick(item)" :show-extra-icon="true"
					:extraIcon="{type:item.icon,color:'#ffffff'}">
					<template v-slot:footer>
						<view v-if="item.showBadge" class="item-footer">
							<text class="item-footer-text" style="color: #FFFFFF;">{{item.rightText}}</text>
...
<style lang="scss" scoped>
	...
	/deep/ .uni-list-item {
		background : url(https://z3.ax1x.com/2021/11/23/op1rzn.gif);
	}
